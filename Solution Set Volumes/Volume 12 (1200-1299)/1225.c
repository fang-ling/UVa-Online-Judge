 ...
 * 2. Count the number of times each digit (0 to 9) appears in the sequence.
 */
...
void main_1225(void) {
  ...
  array_init(&sequence, sizeof(char));
  char num_str[5 + 1];
  int ans[10];
  ...
  while (T--) {
    var N = 0;
    scanf("%d", &N);
    array_remove_all(&sequence);

    /* Construct the sequence */
    var i = 0;
    for (i = 1; i <= N; i += 1) {
      sprintf(num_str, "%d", i);
      var count = strlen(num_str);
      var j = 0;
      for (j = 0; j < count; j += 1) {
        array_append(&sequence, &num_str[j]);
      }
    }
    for (i = 0; i < 10; i += 1) {
      ans[i] = 0;
