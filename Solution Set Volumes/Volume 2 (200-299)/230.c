...
//  hxy
//
//  Created by Fang Ling on 2024/4/26.
//
...
static void red_black_tree_predecessor(
  struct RedBlackTree* tree,
  void* key,
  void* result
) {
...
static Int32 compare(const void* lhs, const void* rhs) {
  var a = (struct Book*)lhs;
  var b = (struct Book*)rhs;
  
  var result = string_compare_ascii(a->author, b->author);
  if (result == 0) {
    return string_compare_ascii(a->title, b->title);
  }
  return result;
}

static void borrow(
  struct RedBlackTree* shelf,
  struct String* title,
  struct RedBlackTree* trash
) {
  struct Book book;
  var i = 0;
  for (i = 1; i <= shelf->count; i += 1) {
    red_black_tree_select(shelf, i, &book);
    if (string_compare_ascii(title, book.title) == 0) {
      break;
    }
  }
  red_black_tree_remove(shelf, &book);
  red_black_tree_insert(trash, &book);
}

static void return_book(
  struct RedBlackTree* desk,
  struct String* title,
  struct RedBlackTree* trash
) {
  struct Book book;
  var i = 0;
  for (i = 1; i <= trash->count; i += 1) {
    red_black_tree_select(trash, i, &book);
    if (string_compare_ascii(title, book.title) == 0) {
      break;
    }
  }
  red_black_tree_remove(trash, &book);
  red_black_tree_insert(desk, &book);
}
...
static void shelve(
  ...
  struct Book pred;
  while (!desk->is_empty) {
    red_black_tree_min(desk, &book);
    red_black_tree_predecessor(shelf, &book, &pred);
    if (pred.title == NULL || pred.author == NULL) {
      string_c_string(book.title, buf);
      printf("Put %s first\n", buf);
    } else {
      string_c_string(book.title, buf);
      string_c_string(pred.title, buf2);
      printf("Put %s after %s\n", buf, buf2);
    }
    red_black_tree_insert(shelf, &book);
    red_black_tree_remove(desk, &book);
  }
}

void main_230(void) {
  var shelf = red_black_tree_init(sizeof(struct Book), false, compare);
  var desk = red_black_tree_init(sizeof(struct Book), false, compare);
  var trash = red_black_tree_init(sizeof(struct Book), false, compare);
  
  var list_separator = string_init(" by ");
  var split_buf = array_init(sizeof(struct String*));
  struct Book book;
  ...
  var linelen = 0;
  while ((linelen = (int)getline(&line_buf, &linecap, stdin)) > 0) {
    /* Check if at the end of book list */
    if (line_buf[0] == 'E' && line_buf[1] == 'N' && line_buf[2] == 'D') {
      break;
    }
    
    line_buf[linelen - 1] = '\0'; /* Remove the tailling \n */
    /* Split the input string */
    var line_buf_string = string_init(line_buf);
    string_components(line_buf_string, list_separator, split_buf);
    string_deinit(line_buf_string);
    /* Save the book to shelf */
    array_get(split_buf, 0, &(book.title));
    array_get(split_buf, 1, &(book.author));
    red_black_tree_insert(shelf, &book);
    array_remove_all(split_buf);
  }
  
  /* Read record list */
  while ((linelen = (int)getline(&line_buf, &linecap, stdin)) > 0) {
    /* Check if at the end of record list */
    if (line_buf[0] == 'E' && line_buf[1] == 'N' && line_buf[2] == 'D') {
      break;
    }
    ...
    if (line_buf[0] == 'B' && line_buf[1] == 'O') { /* BORROW */
      char* _s = line_buf;
      _s += 7;
      var title = string_init(_s);
      borrow(shelf, title, trash);
      string_deinit(title);
    } else if (line_buf[0] == 'R' && line_buf[1] == 'E') { /* RETURN */
      char* _s = line_buf;
      _s += 7;
      var title = string_init(_s);
      return_book(desk, title, trash);
      string_deinit(title);
    } else {
      shelve(shelf, desk);
      printf("END\n");
    ...
    printf("DESK:\n");
    for (i = 1; i <= desk->count; i += 1) {
      red_black_tree_select(desk, i, &book);
      string_c_string(book.title, buf);
      printf("  %s\n", buf);
    }
    printf("TRASH:\n");
  ...
  /* Still have memory leak, string are not deinitialized. */
  red_black_tree_deinit(shelf);
  red_black_tree_deinit(desk);
  red_black_tree_deinit(trash);
  array_deinit(split_buf);
  free(list_separator);
}
