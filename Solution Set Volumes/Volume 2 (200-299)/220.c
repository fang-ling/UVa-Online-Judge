...
//  hxy
//
//  Created by Fang Ling on 2024/1/19.
//
 ...
 * element.
 */
static bool b_tree_contains(struct BTree* tree, void* key) {
  var i = 0;
  if (
    _b_tree_search(
      (*tree).root,
...
static int compare_220(const void* a, const void* b) {
  var sa = *(struct FixedLengthString*)a;
  var sb = *(struct FixedLengthString*)b;
  return strcmp(sa.str, sb.str);
}

static void get_available_position_220(
  char board[8][8],
  bool current_player,
  struct BTree* res
) {
  var disk_color = (char)(current_player ? 'W' : 'B');
  var another_color = (char)(current_player ? 'B' : 'W');
  var pos_i = 0;
  var pos_j = 0;
...
static void make_a_move_220(
  ...
  /* Find left up */
  var j = c - 1;
  for (k = r - 1; k >= 0 && j >= 0; k -= 1, j -= 1) {
    if (board[k][j] == disk_color) {
      var i = 0;
      var l = c - 1;
      for (i = r - 1; i > k && l > j; i -= 1, l -= 1) {
...
void main_220(void) {
  ...
  while (T--) {
    ...
    scanf("%s", op);
    /* true: W;  false: B */
    var current_player = op[0] == 'W';
    while (true) {
      b_tree_init(
        &position,
        1024,
        sizeof(struct FixedLengthString),
        false,
        compare_220
      );
      ...
      scanf("%s", op);
      if (op[0] == 'Q') {
        b_tree_deinit(&position);
        break;
      } else if (op[0] == 'L') {
        get_available_position_220(board, current_player, &position);
        if (position.count > 0) {
          var count = position.count;
          for (i = 0; i < count; i += 1) {
            struct FixedLengthString buf;
            b_tree_min(&position, &buf);
            printf("%s", buf.str);
            b_tree_remove(&position, &buf);
            if (i != count - 1) {
              printf(" ");
            }
          }
        ...
        buf.str[4] = ')';
        buf.str[5] = '\0';
        if (position.count == 0 || !b_tree_contains(&position, &buf)) {
