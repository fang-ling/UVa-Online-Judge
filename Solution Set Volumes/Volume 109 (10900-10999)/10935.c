...
void main_10935(void) {
  struct Array ans;
  struct Deque list;
  array_init(&ans, sizeof(int));
  deque_init(&list, sizeof(int));
  
  var n = 0;
  while (scanf("%d", &n) == 1 && n != 0) {
    array_remove_all(&ans);
    deque_remove_all(&list);
    ...
    while (list.count >= 2) {
      var delta = 0;
      deque_get(&list, 0, &delta);
      deque_remove_first(&list);
      array_append(&ans, &delta);
      deque_get(&list, 0, &delta);
      deque_remove_first(&list);
      deque_append(&list, &delta);
    }
    printf("Discarded cards:%s", ans.is_empty ? "" : " ");
    for (i = 0; i < ans.count; i += 1) {
      var delta = 0;
      array_get(&ans, i, &delta);
      printf("%d%s", delta, i == ans.count - 1 ? "" : ", ");
    }
    var delta = 0;
    deque_get(&list, 0, &delta);
    printf("\nRemaining card: %d\n", delta);
