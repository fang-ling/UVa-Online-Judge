...
//  hxy
//
//  Created by Fang Ling on 2024/2/14.
//
...
static int cmp_815(const void* a, const void* b) {
  if (*(int*)a < *(int*)b) {
    return -1;
  } else if (*(int*)a > *(int*)b) {
    return 1;
...
void main_815(void) {
  ...
  while (scanf("%d %d", &n, &m) == 2 && n != 0 && m != 0) {
    ...
    for (i = 0; i < len; i += 1) {
      var delta = 0;
      scanf("%d", &delta);
      array_append(&regions, &delta);
    }
    
    var water = 0;
    scanf("%d", &water);
    array_sort(&regions, cmp_815);
    var water_count = 0;
    var water_level = 0.0;
    /*array_get(&regions, 0, &water_level); //dont have same memory layout */
    water_level = ((int*)regions._storage)[0];
    for (i = 0; i < regions.count; i += 1) {
      if (water > 0) {
        if (i != regions.count - 1) { /* not the highest region */
          var _a = 0;
          var _b = 0;
          array_get(&regions, i + 1, &_a);
          array_get(&regions, i, &_b);
          var delta = _a - _b;
          var alpha = 10 * (i + 1) * 10 * delta;
          if (water < alpha) {
            water_level += (double)water / 10.0 / (10.0 * (i + 1));
            water -= water;
          } else {
            water_level += delta;
            water -= alpha;
    ...
    var unsafe_cast = (int*)regions._storage;
    for (i = 0; i < regions.count; i += 1) {
      if (water_level > unsafe_cast[i]) {
        water_count += 1;
      }
    }
    printf("Region %d\n", region_count += 1);
    ...
    printf(
      ...
      (double)water_count / regions.count * 100
    );
  }
  
  array_deinit(&regions);
}
