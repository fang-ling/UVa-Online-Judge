...
static bool b_tree_predecessor(struct BTree* tree, void* key, void* result) {
  var i = 0;
...
static bool b_tree_successor(struct BTree* tree, void* key, void* result) {
  var i = 0;
  var key_node = _b_tree_search(
    (*tree).root,
    key,
...
static int cmp_508(const void* a, const void* b) {
  var ea = *(struct Entry*)a;
  var eb = *(struct Entry*)b;
  return strcmp(ea.key.s, eb.key.s);
}

void main_508(void) {
  char C[256];
  char buf[256];
  struct String tbl[256];
  struct String cipher;
  struct String delta;
  struct BTree dict;
  struct Entry entry;
  b_tree_init(&dict, 1024, sizeof(struct Entry), false, cmp_508);
  ...
  while (true) {
    ...
    scanf("%s", buf);
    strcpy(tbl[C[0]].s, buf);
    tbl[C[0]].count = (int)strlen(buf);
  }
  while (true) {
    scanf("%s", C);
    if (strlen(C) == 1 && C[0] == '*') {
