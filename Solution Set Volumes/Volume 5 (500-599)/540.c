...
//  hxy
//
//  Created by Fang Ling on 2024/4/9.
//
...
static void enqueue(
  int* daa,
  struct Deque* queues,
  struct Deque* team_queue,
  int element
) {
  if (queues[daa[element]].is_empty) {
    deque_append(team_queue, &daa[element]);
  }
  deque_append(&queues[daa[element]], &element);
}
...
static int dequeue(
  ...
  var first = 0;
  deque_get(team_queue, 0, &first);
  var res = 0;
  deque_get(&queues[first], 0, &res);
  deque_remove_first(&queues[first]);
  if (queues[first].is_empty) {
    deque_remove_first(team_queue);
  }
  return res;
}
...
void main_540(void) {
  /* Direct Access Array */
  var daa = (int*)malloc(sizeof(int) * 1000000);
  var scenario = 0;
  char op[8];
  struct Deque queues[1000];
  struct Deque team_queue;
  deque_init(&team_queue, sizeof(int));
  ...
  while (scanf("%d", &t) == 1 && t != 0) {
    ...
    deque_remove_all(&team_queue);
    
    for (i = 0; i < t; i += 1) {
      deque_init(&queues[i], sizeof(int));
      var n = 0;
      scanf("%d", &n);
      while ((n -= 1) >= 0) {
        var j = 0;
    ...
    while (scanf("%s", op) == 1 && op[0] != 'S' && op[1] != 'T') {
      if (op[0] == 'E') {
        var elem = 0;
        scanf("%d", &elem);
        enqueue(daa, queues, &team_queue, elem);
      } else if (op[0] == 'D') {
        printf("%d\n", dequeue(daa, queues, &team_queue));
      }
    }
    printf("\n");
    
    for (i = 0; i < t; i += 1) {
      deque_deinit(&queues[i]);
    }
  }
  deque_deinit(&team_queue);
}
