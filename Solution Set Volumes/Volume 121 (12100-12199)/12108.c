...
//  hxy
//
//  Created by Fang Ling on 2024/1/26.
//
...
void main_12108(void) {
  struct Array students;
  array_init(&students, sizeof(struct Student));
  
  var case_num = 0;
  struct Student delta;
  var n = 0;
  while (scanf("%d", &n) == 1 && n != 0) {
    array_remove_all(&students);
    
    var i = 0;
    for (i = 0; i < n; i += 1) {
      scanf("%d %d %d", &delta.a, &delta.b, &delta.c);
      delta.is_sleep = false;
      array_append(&students, &delta);
    }
    var has_solution = false;
    var t = 0;
    for (t = 0; t <= 100000; t += 1) {
      for (i = 0; i < students.count; i += 1) {
        array_get(&students, i, &delta);
        if (
          (t + delta.c) % (delta.a + delta.b) <= delta.a &&
          (t + delta.c) % (delta.a + delta.b) != 0
        ) {
          delta.is_sleep = false;
          array_set(&students, i, &delta);
      ...
      var b = 0;
      var unsafe_cast = (struct Student*)students._storage;
      for (i = 0; i < students.count; i += 1) {
        b += unsafe_cast[i].is_sleep ? 1 : 0;
      }
      if (b <= n / 2) {
        has_solution = true;
      }
